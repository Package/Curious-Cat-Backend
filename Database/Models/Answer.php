...
class Answer
{
     ...
     */
    public static function create(int $questionID, array $user, string $label)
    {
        // Validate input
        ...
        $db = Database::connect();
        $statement = $db->prepare("INSERT INTO answers (question_id, label, created_at, user_id)
     ...
     */
    public static function delete(int $answerID, array $user)
    {
        // Validate input
        if (!$answerID || !$user['id']) {
            throw new OperationFailedException("An error occurred. Please login and try again.");
        ...
        $db = Database::connect();
        $statement = $db->prepare("DELETE FROM answers WHERE id = :answer_id AND user_id = :user_id");
