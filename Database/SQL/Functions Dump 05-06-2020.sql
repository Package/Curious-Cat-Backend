CREATE OR REPLACE FUNCTION public.fn_stats_newest()
 RETURNS TABLE(id integer, username character varying, counter timestamp without time zone)
 LANGUAGE sql
AS $function$
...
CREATE OR REPLACE FUNCTION public.fn_stats_questions()
 RETURNS TABLE(id integer, username character varying, created_at timestamp without time zone, counter bigint)
 LANGUAGE sql
AS $function$
...
CREATE OR REPLACE FUNCTION public.fn_notification_get(userid integer)
 RETURNS TABLE(id integer, label character varying, notification_read smallint, user_id integer, created_at timestamp without time zone, from_user integer, from_username character varying, notification_type smallint, notification_type_string character varying, context character varying, context_id integer, hidden smallint)
 LANGUAGE sql
AS $function$
    SELECT
        n.id,
...
CREATE OR REPLACE FUNCTION public.fn_profile_info(profileid integer, currentuser integer)
 RETURNS TABLE(follower_count bigint, following_count bigint, user_id integer, username character varying, created_at timestamp without time zone, is_following boolean, own_profile boolean)
 LANGUAGE sql
AS $function$
    SELECT
...
CREATE OR REPLACE FUNCTION public.fn_search_question(searchquery character varying, searchlimit smallint)
 RETURNS TABLE(question_id integer, question_label character varying, question_timestamp timestamp without time zone, question_name_hidden smallint, from_user integer, from_user_name character varying, target_user integer, target_user_name character varying, answer_id integer, answer_label character varying, answer_timestamp timestamp without time zone)
 LANGUAGE sql
AS $function$
SELECT
    q.id AS question_id,
